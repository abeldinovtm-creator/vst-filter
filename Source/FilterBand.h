...
// Не буквально "неограниченно" (APVTS требует фиксированный набор параметров,
// объявленных заранее — динамически добавлять/удалять полосы во время работы
// нельзя без полной переделки хранения состояния), но щедрый фиксированный
// пул — практически столько же, сколько у большинства конкурентов (ФабФильтр
// Pro-Q — до 24 полос).
