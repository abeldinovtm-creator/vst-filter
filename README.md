# FabEQ — параметрический EQ (JUCE, VST3/AU)

8-полосный параметрический эквалайзер в духе FabFilter Pro-Q:
- Drag-точки прямо на графике АЧХ (клик и перетаскивание = частота/gain, колесо мыши = Q, двойной клик = вкл/выкл полосы)
- Живой спектр-анализатор фона (FFT, post-EQ)
...
VST3 появится в:
`build/FabEQ_artefacts/Release/VST3/FabEQ.vst3`
...
`build/FabEQ_artefacts/Release/VST3/FabEQ.vst3`
`build/FabEQ_artefacts/Release/AU/FabEQ.component`
...
└── Source/
    ├── PluginProcessor.h/.cpp  — DSP-движок, параметры (APVTS), 8 полос EQ, FFT для анализатора
...
- Спектр pre/post EQ раздельно (сейчас только post)
- Mid/Side и Stereo/L/R режимы на полосу
- Пресеты (JUCE `AudioProcessorValueTreeState` уже поддерживает сериализацию — 
  остаётся добавить `PresetManager` с сохранением `.xml` в папку пользователя)
